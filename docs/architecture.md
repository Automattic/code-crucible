...
For `allowlist`, `mock`, and `replay` runs, evaluator environments include `CRUCIBLE_HTTP_FIXTURES`, `CRUCIBLE_MOCK_GATEWAY_SOURCE`, `CRUCIBLE_MOCK_GATEWAY_ADDR`, `CRUCIBLE_MOCK_GATEWAY_URL`, standard proxy variables such as `HTTP_PROXY` and `HTTPS_PROXY`, and mode-specific values such as `CRUCIBLE_ALLOWED_HOSTS` or test CA variables. In container mode, `evaluator/resource-wrapper.sh` starts that gateway before invoking `evaluator.sh`. The current gateway is generated Go source, so the sandbox image must include `go` until Code Crucible ships a packaged gateway binary.
