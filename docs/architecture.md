...
`crucible evaluator generate` builds an evaluator-generation prompt at `prompts/evaluator-generation.md`, invokes the selected provider, and asks it to replace `evaluator/evaluator.sh` plus write optional design notes at `evaluator/evaluator.md`. The drafted evaluator is then run against the baseline candidate with validation outputs archived under `tmp/evaluator-validation/` and summarized in `evaluator/validation.json`. Only a baseline-passing evaluator marks `run.json` with `evaluator_generated: true` and moves any `needs-evaluator` candidates to `pending` so normal evaluation can begin. The provider invocation, stdout, stderr, and final message are archived under `agents/` using the same provider contract as generation.
