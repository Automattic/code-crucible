...
Command providers are invoked with the prompt on stdin and receive environment variables for the provider name, project directory, run directory, prompt path, final-response path, and model override. Their stdout, stderr, final response, and invocation metadata are archived under the same `agents/` directory. The repository includes a Claude Code command-provider example in `examples/agent-providers/claude-code-config.json`; it uses Claude Code print mode as a configured `kind: command` provider rather than adding Claude-specific branches to the shared tournament code.
...
`crucible run --auto` is the lowest-prompt setup path. It creates a local discovery plan, selects the highest-ranked source-path suggestion, creates the run archive, generates and validates an evaluator through the configured provider, records the baseline validation as normal candidate metrics, and prints the leaderboard. Raw provider stdout and stderr are still archived under the run `agents/` directory, but they are not streamed during `--auto` so the terminal remains focused on status and results. `--auto --generate --evaluate` extends that same command into competitor generation and evaluation. `crucible run --generate` remains a narrower shortcut: it creates the run archive first, then calls the selected generation provider with the newly-created run ID. The separate `run`, `evaluator generate`, `generate`, and `evaluate` commands remain the safer workflow when the operator wants to review or edit interface docs, evaluator scripts, or prompts before spending model runs.
...
`crucible evaluator generate` builds an evaluator-generation prompt at `prompts/evaluator-generation.md`, invokes the selected provider, and asks it to replace `evaluator/evaluator.sh` plus write optional design notes at `evaluator/evaluator.md`. The drafted evaluator is then run against the baseline candidate with validation outputs archived under `tmp/evaluator-validation/` and summarized in `evaluator/validation.json`. Only a baseline-passing evaluator marks `run.json` with `evaluator_generated: true`, records the validation as the baseline's normal metrics and verdict artifacts, and moves any other `needs-evaluator` candidates to `pending` so normal evaluation can begin. The provider invocation, stdout, stderr, and final message are archived under `agents/` using the same provider contract as generation.
...
After evaluation produces a winner, `crucible promote` copies a selected candidate from the run archive back into the original project `source_path`. With no candidate argument, it chooses the best passing non-baseline result. Promotion is intentionally separate from adoption: adoption records generated artifacts in `leaderboard.json`, while promotion modifies the host project. File source paths replace the matching file from the candidate `src/`; directory source paths overlay regular files without deleting unrelated project files. Promotions reject targets outside the project or inside `.crucible/`, require passed candidates by default, support `--dry-run`, and archive a JSON report under `promotions/`.
