...
Currently, `deny` mode is enforced only for Docker or Podman evaluation with `--sandbox-network none`. `allowlist`, `mock`, `replay`, and `record` modes can be `partial` when Code Crucible starts the gateway and exports proxy environment variables. Clients that ignore proxy variables can use direct-routed gateway URLs when their base URL is configurable; raw sockets and clients that cannot be redirected still require evaluator-specific configuration. Local gateway-backed evaluations include warnings because local mode cannot block or transparently intercept raw socket traffic.
