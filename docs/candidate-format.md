...
If `verdict.json` is missing or invalid, Code Crucible marks the candidate as failed. A new baseline starts as `needs-evaluator` when no evaluator command or script is configured, because there is no work in progress and no reliable way to produce metrics yet. `crucible evaluator generate` can ask the configured provider to replace `evaluator/evaluator.sh`; Code Crucible validates that drafted evaluator against the baseline and writes `evaluator/validation.json`. After successful validation, Code Crucible marks the run with `evaluator_generated: true`, records the passing baseline validation as normal baseline metrics and verdict artifacts, and moves any other waiting candidates to `pending`. A run with a supplied evaluator starts the baseline as `pending` until `crucible evaluate` runs. If a TUI evaluation is canceled before an unevaluated candidate completes, Code Crucible marks that affected candidate as `canceled` and records a cancellation event under `events/cancellations.jsonl`. Completed `passed` and `failed` candidates are not overwritten by cancellation. If generation is canceled, valid unadopted candidate directories from the active round are recorded in `leaderboard.json` with `canceled` status, while partial candidate directories are listed in the cancellation event.
